...
let find_replace_occurs_lb name new_name k (id, args, e) =
  let* e' = find_replace_occurs_aexpr name new_name k e in
  let* id =
...
let find_replace_occurs_lb_if_needed name new_name k lb =
  let need_to_replace (id, args, _) =
    identifier_to_str id = name || List.mem name (map_ident_to_str args)
  in
  if need_to_replace lb || true
  then find_replace_occurs_lb name new_name k lb
  else find_replace_occurs_lb "" "" k lb
;;

let find_replace_occurs_decl name new_name k = function
  | ADSingleLet (rec_flag, lb) ->
    let* lb' = find_replace_occurs_lb_if_needed name new_name k lb in
  ...
  | ADMutualRecDecl (Recursive, lb1, lb2, lb_tl) as dlet ->
    ...
    then return dlet
      ...
      let* lb1' = find_replace_occurs_lb_if_needed name new_name k lb1 in
      let* lb2' = find_replace_occurs_lb_if_needed name new_name k lb2 in
      let* lb_tl' = map (find_replace_occurs_lb_if_needed name new_name k) lb_tl in
      return @@ ADMutualRecDecl (Recursive, lb1', lb2', lb_tl')
  | ADMutualRecDecl (Not_recursive, lb1, lb2, lb_tl) as dlet ->
    let names = get_names_of_decl dlet in
    if List.mem name names
    then return dlet
    else
      let* lb1' = find_replace_occurs_lb name new_name k lb1 in
      let* lb2' = find_replace_occurs_lb name new_name k lb2 in
      let* lb_tl' = map (find_replace_occurs_lb name new_name k) lb_tl in
      return @@ ADMutualRecDecl (Not_recursive, lb1', lb2', lb_tl')
;;

let find_replace_occurs_decls name new_name k prog =
  let rec helper acc = function
    | decl :: tl ->
      let* decl' = find_replace_occurs_decl name new_name k decl in
    ...
    | decl :: decls ->
      ...
      let rec fold_over_idents decl decls = function
        | name :: tl ->
          ...
          let* decl' = find_replace_occurs_decl name new_name None decl in
          let* new_name = gen_name name TopLevel in
          let* decls' = find_replace_occurs_decls name new_name None decls in
